...
late VideoPlayerController _videoPlayerController;
  ...
  void initState() {
    // showNotification();
    var url = "https://media.publit.io/file/tableau/Video-5-How-to-prepare-Line-Charts-using-Tableau.mp4";
    ...
    _videoPlayerController.addListener(() {
      ...
      setState(() {
        lastPlayed = _videoPlayerController.value.position.inSeconds.toInt();
      });
    });
    fetchCourses();
  ...
  getProfile()async{
    final url= "https://us-central1-fir-messaging-b5ab7.cloudfunctions.net/helloWorld";
    final response = await http.get(Uri.parse(url),
        // body:{"uid":"VRsprwlXQia34O91DJxA7BI4oQu2","cid":"abcd","utype":"J"}
    );
    print("...getprofile....");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(onPressed: ( ) async {
        setState(() {
        print("srinu");
        total = _videoPlayerController.value.duration.inSeconds.toInt();
        print(_videoPlayerController.value.duration.inMicroseconds.bitLength.toString());
          print((_videoPlayerController.value.duration.inSeconds).toInt().toString());
          ...
          return SingleChildScrollView(
            child: Column(
              children: [
                ...
                //   controller = await VideoPlayerController.network('https://media.publit.io/file/tableau/Video-5-How-to-prepare-Line-Charts-using-Tableau.mp4');
                //   print("length ${controller.value.duration.toString()}");
                //   // controller.play();
                // }, child: Text("Play")),
                // Text(controller.value.duration.toString()),
                total!=0?Container(child: LinearProgressIndicator(
                  value: ((lastPlayed/total)*100)/100,
                  valueColor: new AlwaysStoppedAnimation<Color>(Colors.red),
                  backgroundColor: Colors.blue,
                  minHeight: 50,
                ),height: 100):Container(child: LinearProgressIndicator(
                  value: 0,
                  valueColor: new AlwaysStoppedAnimation<Color>(Colors.red),
                  backgroundColor: Colors.blue,
                  minHeight: 50,
                ),height: 100),
                total!=0?Text(((lastPlayed/total)*100).toInt().toString()+"%"):Text("0%"),
                Container(
                  width: screenWidth,
