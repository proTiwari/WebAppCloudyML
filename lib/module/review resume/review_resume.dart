            ...
            children: [
              ...
              Padding(
                padding: EdgeInsets.symmetric(
                    horizontal: width / 5, vertical: height / 50),
                                ...
                                child: Center(
                                  child: Row(
                                    ...
                                    children: [
                                     ...
                                     Row(
                                    children: [
                                      studentData[index].comment.isNotEmpty ?
                                        Icon(Icons.check_box, color: Colors.green,) : SizedBox(),
    ...
    try{
      await FirebaseFirestore.instance.collection('StudentResumes').get().then((value) {
        ...
        value.docs.forEach((element) {
          ...
          studentData.add(
              StudentResumeData(name: element.get('name'), resumeLink: element.get('link'), email: element.get('email'), id: element.get('id'), comment: element.get('comments'), submitDate: element.get('submitDate')));
        });
List<StudentResumeData> sortedData = List.from(studentData);
        sortedData.sort((a, b) => b.submitDate.compareTo(a.submitDate));
        studentData.clear;
...
class StudentResumeData {
 ...
 Timestamp get submitDate => _submitDate;
}
