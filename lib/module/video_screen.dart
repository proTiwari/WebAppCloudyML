    ...
    for (var t in data.entries) {
      ...
      for (var name in t.value) {
        ...
        await FirebaseFirestore.instance
          ...
          // print(value.docs.length);
        }).catchError((err)=>print("Erroror ${err}"));
      }
      listOfVideo = [];
    }
    print("done = $done");
  ...
  Future<void> getCourseData() async {
    ...
    await FirebaseFirestore.instance
      ...
      for (var i in sectionName) {
        print("-0-0-0- ${curriculumdata[i]!=null}");
          dic[i] = i;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: ()async{
          await FirebaseFirestore.instance.collection('courseprogress').doc(_auth.currentUser!.uid).get().then((value) {
            if(value.exists)
              {
                var coursedetail = value.data()![courseName];
                print(coursedetail);
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        icon: Icon(
                          menuClicked ? Icons.fullscreen_rounded : Icons.fullscreen_exit_rounded,
                          color: Colors.white,
                        ),
                      ),
                    ],
